...
function M.setup_hooks(plugin_id, config)
  ...
  if (config.defer_verification) then
    ...
    -- register_listener("before", "event_initialized", function (session, info, body)
    --   print('got here lol')
    --   -- print(require("user.utils").dump(dap_breakpoints.get_breakpoint_signs()))
    -- end)
  end
